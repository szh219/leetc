  ...
  <div class="va-chat">
    ...
    <div class="va-chat__controls">
      <va-input
        ...
        @keypress.enter="sendMessage;getsearch"
        placeholder="Type your message..."
        class="va-chat__input mr-2"
      />
      <va-button @click="sendMessage();getsearch()">
        {{ $t("chat.sendButton") }}
      </va-button>
    </div>
  </div>
...
// import mypage1 from "../../../../../../vuestic-admin-master/src/pages/mypages/mypage1"
//
// Vue.use(Router)
...
export default {
  ...
  methods: {
    sendMessage() {
      ...
      // axios.post(path, '/search/'+payload).then(
      // res => {
      ...
      //console.log(this.getMessage);  空值

    },
    getsearch() {
      const payload = {
        words: this.inputMessage
      }
      const path = 'http://localhost:5000/search/'; // '/api/'
      console.log(path+this.inputMessage)

      axios.get(path+this.inputMessage, ).then(
      res => {
      this.getMessage = res.data
        console.log(this.getMessage)
         this.$emit(
  ...
  //       }),
      ...
      this.getMessage = "";
      //alter('Success' + ',' + this.inputMessage);
    }
  },
  computed: {
    theme() {
